...
using Queue = cms::StaticQueue<NothrowElement, 4>;

} // namespace

static_assert(
    std::is_default_constructible<cms::StaticQueue<NonDefault, 1>>::value,
...
static_assert(
    std::is_same<decltype(std::declval<Queue&>().pop()), cms::Status>::value,
    "pop has the wrong return type");
static_assert(
    std::is_same<decltype(std::declval<Queue&>().clear()), void>::value,
    "clear has the wrong return type");
static_assert(
    std::is_same<decltype(std::declval<Queue&>().emplace(1)), cms::Status>::value,
...
using ThrowingQueue = cms::StaticQueue<ThrowingElement, 2>;
...
#if SIZE_MAX >= 255
static_assert(
    std::is_same<cms::detail::small_index_t<255>, std::uint8_t>::value,
    "255 must fit uint8_t");
#endif

#if SIZE_MAX >= 256
static_assert(
    std::is_same<cms::detail::small_index_t<256>, std::uint16_t>::value,
    "256 requires uint16_t");
#endif

#if SIZE_MAX >= 65535
static_assert(
    std::is_same<cms::detail::small_index_t<65535>, std::uint16_t>::value,
    "65535 must fit uint16_t");
#endif

#if SIZE_MAX >= 65536
static_assert(
    std::is_same<cms::detail::small_index_t<65536>, std::uint32_t>::value,
