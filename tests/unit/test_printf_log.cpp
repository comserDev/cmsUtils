...
int main() {
    ...
    CMS_TEST_CHECK(logger.wouldLog(cms::log::Level::trace));
    CMS_TEST_CHECK(logger.wouldLog(static_cast<cms::log::Level>(0xFF)));

    clockState.current = 1;
    CMS_TEST_CHECK(cms::log::logf(
        logger, cms::log::Level::info, "literal") == cms::Status::ok);
    CMS_TEST_CHECK(logger.drainOne() == cms::Status::ok);
    checkLine(sinkState, 0, "[1] [INFO] literal\n");

    clockState.current = 2;
    CMS_TEST_CHECK(cms::log::logf(
        logger, cms::log::Level::info, "%d", 42) == cms::Status::ok);
    CMS_TEST_CHECK(logger.drainOne() == cms::Status::ok);
    ...
    CMS_TEST_CHECK(cms::log::logf(
        logger, cms::log::Level::info, "") == cms::Status::ok);
    CMS_TEST_CHECK(logger.drainOne() == cms::Status::ok);
    ...
    using SmallLogger = cms::log::AsyncLogger<
        16,
        2,
        CountingClock,
        CapturingSink,
    ...
    CMS_TEST_CHECK(cms::log::logf(
        ...
        cms::log::Level::info,
        "%s",
        "123456789012345") == cms::Status::ok);
    CMS_TEST_CHECK(smallClock.calls == 1);
    CMS_TEST_CHECK(small.pending() == 1);
    CMS_TEST_CHECK(small.drainOne() == cms::Status::ok);
    ...
    CMS_TEST_CHECK(cms::log::logf(
        ...
        cms::log::Level::info,
        "%s",
        "1234567890123456") == cms::Status::no_space);
    CMS_TEST_CHECK(cms::log::logf(
        small,
        cms::log::Level::info,
        "%s",
