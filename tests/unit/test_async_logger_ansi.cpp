...
int main() {
    ...
    CMS_TEST_CHECK(logger.log(cms::log::Level::info, source)
        == cms::Status::ok);
    clockState.current = 20;
    CMS_TEST_CHECK(logger.log(cms::log::Level::debug, "one")
        == cms::Status::ok);
    ...
    CMS_TEST_CHECK(logger.log(cms::log::Level::warning, "two")
        == cms::Status::ok);
    CMS_TEST_CHECK(logger.full());
    CMS_TEST_CHECK(logger.drainOne() == cms::Status::ok);
    CMS_TEST_CHECK(logger.drainOne() == cms::Status::ok);
    ...
    using NullAnsiLogger = cms::log::AsyncLogger<
        16,
        1,
        CountingClock,
        CapturingSink,
    ...
    CMS_TEST_CHECK(nullLogger.log(cms::log::Level::error, "null")
        == cms::Status::ok);
    ...
    CMS_TEST_CHECK(plainLogger.log(cms::log::Level::info, "plain")
        == cms::Status::ok);
    CMS_TEST_CHECK(plainLogger.drainOne() == cms::Status::ok);
    checkLine(plainSinkState, 0, "[5] [INFO] plain\n");

    using MaximumAnsiLogger = cms::log::AsyncLogger<
        64,
        1,
        CountingClock,
        CapturingSink,
