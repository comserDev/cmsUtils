# Binary utility API 계약

이 문서의 API는 `cms::util`에서 제공하는 deterministic/zero-heap component다. C++17에서
exception과 RTTI 없이 사용할 수 있으며 OS, transport, protocol schema에 의존하지 않는다.

## Byte storage

- `ByteView(data, size)`는 binary 데이터를 소유하지 않는다. `nullptr` 입력은 빈 view로
  정규화하며 embedded NUL을 보존한다. `subview()`는 끝에서 범위를 제한한다.
- `ByteBuffer(data, capacity, size)`는 caller가 소유하는 storage와 현재 size를 함께 참조한다.
...
입력이 부족하면 `Status::out_of_range`를 반환하며 cursor와 output argument를 변경하지 않는다.

## BinaryWriter

`BinaryWriter(ByteBuffer)`는 대응하는 unsigned big-endian write와 `writeBytes`를
제공한다. 기존 buffer 끝에서 시작하고 성공할 때마다 shared size가 cursor만큼 전진한다.
공간 부족은 `Status::no_space`, 잘못된 buffer는 `Status::invalid_argument`이며 실패 시
buffer 내용, size, writer position을 변경하지 않는다. `writeBytes`는 source와 destination이
겹치는 경우도 지원한다.
...
TLV, frame header, message ID, session, ACK, retry 같은 protocol 의미는 이 API의 범위가 아니다.
