# SHA-256과 HMAC-SHA256

cmsUtils는 Host와 ESP32에서 공통으로 사용할 수 있는 SHA-256과 HMAC-SHA256
one-shot API를 제공한다. 두 구현 모두 C++17 기반이며 동적 메모리, filesystem,
Arduino runtime에 의존하지 않는다.
...
이번 API에는 digest verification helper가 포함되지 않는다. 보안 검증에서는 일반
`memcmp()`을 권장하지 않으며, application이 제공하는 검증된 constant-time 비교
기능을 사용한다. 로그에 key나 digest를 자동으로 출력하지 않는다.

## 결과와 자원 사용 규칙

두 API 모두 caller가 제공한 32-byte 배열에 결과를 기록한다. 입력 검증에 실패하면
기존 digest를 유지한다. 내부에서는 고정 크기 stack buffer와 private SHA-256
streaming state만 사용하며 heap allocation, exception, RTTI가 필요하지 않다.

Public streaming SHA-256/HMAC API, 암호화, random 생성, key derivation은 제공하지
않는다.
