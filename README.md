...
`cmsUtils`는 embedded와 Host 환경에서 함께 사용할 수 있는 C++17 범용 유틸리티
라이브러리다. 고정 용량을 사용하는 deterministic/zero-heap 경로와 동적 메모리를
사용하는 Host 편의 기능을 구분해 제공한다.
...
- `Document` 기반 INI와 Host/ESP32 파일 adapter
...
전체 결과가 들어가지 않으면 destination을 바꾸지 않는 방식으로 동작한다.
...
Deterministic component는 runtime heap allocation이나 Host 전용 의존성을 사용하지 않는다.
여기에는 fixed byte/string/queue storage, binary reader/writer, CRC와 범용 알고리즘이 포함된다.

`StdQueueAsyncLogger`, `StdMutex`, `StdFileSink`, `SystemClock` 같은 Host/platform component는
선택한 backend의 메모리 할당, exception, OS/runtime 규칙을 따른다. 따라서 라이브러리 전체가
항상 zero-heap이라고 일반화할 수 없다.
...
CI는 GCC, Clang, MSVC와 ESP32 빌드 경로를 확인한다.
